import { ArrowUpRight, ExternalLink, Mail, UserRound } from 'lucide-react'
...
const featuredClientWorkIds = ['tangerine', 'ilika', 'futurex', 'ptcgram', 'ptc-group-india', 'ptc-logistics']

export default function Home() {
  const featuredClientWork = featuredClientWorkIds
    .map((id) => clientWork.find((site) => site.id === id))
    .filter(Boolean)
  ...
  return (
    <>
      <Section className="py-12 sm:py-20 lg:py-24">
        <div className="grid items-center gap-10 sm:gap-12 lg:grid-cols-[1.2fr_1fr] lg:gap-20">
          <div className="max-w-xl">
            ...
            <h1 className="mt-2 text-4xl font-semibold leading-[1.08] tracking-[-0.045em] sm:text-6xl lg:text-7xl">
              {personalInfo.name}
            </h1>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Button as="a" href="/client-work" size="lg" icon={ArrowUpRight} className="w-full sm:w-auto">
                View My Work
              </Button>
              <Button as="a" href="/resume" size="lg" variant="outline" className="w-full sm:w-auto">
                View Resume
              </Button>
            </div>
      ...
      <Section>
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          {featuredClientWork.map((site) => (
            <Card key={site.id} hoverable as="a" href={`/client-work/${site.id}`}>
              ...
              {site.tech.length > 0 && (
                <div className="mt-4 flex flex-wrap gap-2">
                  {site.tech.map((tech) => <TechTag key={tech}>{tech}</TechTag>)}
                </div>
              )}
        ...
        <div className="mt-8">
          <Button as="a" href="/client-work" icon={ArrowUpRight}>
